...
export const PhotoAlbum: React.FC<PhotoAlbumProps> = ({
  ...
  // Empty album — never a dead end, just a book waiting to be filled.
  if (!photo) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
      ...
      >
        <div className="bg-white rounded-3xl p-8 shadow-xl border border-[#E3DFC2] max-w-md">
          <p className="text-5xl mb-4" aria-hidden="true">📷</p>
          <h2 className="text-2xl font-extrabold text-[#2C2C2A]">Our Photo Album</h2>
          <p className="mt-3 text-lg text-[#5E5D57] font-medium leading-relaxed">
            Your family's photos will live here. Any picture you share with {companionName} is
            kept safe in this book.
          </p>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="mt-8 inline-flex items-center gap-2 rounded-full px-7 py-4 text-lg font-extrabold text-white bg-[#3A5D45] hover:bg-[#2B4633] shadow-lg transition-all active:scale-95"
        >
          <X className="w-5 h-5" />
